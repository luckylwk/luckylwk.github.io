...
function kMeans(){
    ...
    var numberCentroids = $('input[name=nCentroids]').val(),
        numberPoints = $('input[name=nPoints]').val();
    
    var width = 600,
        height = 500;

    // Generate the datapoints that we need.
    function generateCluster(nPoints){
        var points = [],
            centroid = [ Math.random() * width, Math.random() * height ];
        points.push( centroid );
        for( var i=0; i<nPoints; i++ ){
            points.push( [ centroid[0] + Math.random() * 100, centroid[1] + Math.random() * 100 ] ); 
        }
        return points;
    }
    dataPoints = generateCluster(numberPoints);




    // All data is available, start creating and drawing the chart.

    // Add the svg-canvas.
    var svg = d3.select('#svg-placeholder').append("svg").attr("id","svg-chart")
        .attr("width", width)
        .attr("height", height);
    ...
    // All the static data has been drawn. Start KMeans iterations.
