function start_kmeans( $div_id, $svg_id ){
    ...
    var voronoi = d3.geom.voronoi().clipExtent( [ [padding, padding], [ width - padding, height - padding ] ] );
	...
	// Create SVG element
	var svg = d3.select($div_id).append("svg").attr("class","chart").attr("id",$svg_id)
				.attr("width", width)
				.attr("height", height);
    ...
    function get_random( type ){
        if( type=='pivot' ){
            tmp_x = Math.random() * (width - 2*margin.random - 2*padding) + margin.random + padding;
            tmp_y = Math.random() * (height - 2*margin.random -2*padding) + margin.random + padding;
        }
        else if( type=='data' ){
            // Generate X-coor.
            tmp_x = xy_piv[0] + (Math.random()-0.5)*150;
            // Make sure the points fall within the graph.
            tmp_x = Math.max( padding, Math.min( tmp_x, width - padding ) );
            // Generate Y-coor.
            tmp_y = xy_piv[1] + (Math.random()-0.5)*150;
            // Make sure the points fall within the graph.
            tmp_y = Math.max( padding, Math.min( tmp_y, height - padding ) );
        }
        return [tmp_x, tmp_y];
    }
    ...
    function gen_random(){
        ...
        svg.select("g.pivot").transition().duration(500).style("opacity",0.05);
    ...
    function init_kmeans(){
        ...
        svg.append("g").attr("class","kmeansvoronoi")
            .selectAll("path")
